module cetus_stable_farming::rewarder {
    ...
    use cetus_stable_farming::config::{Self, OperatorCap, GlobalConfig, checked_package_version, AdminCap};

    const REWARD_PRECISION: u128 = 1000000000000;

    public struct PoolRewarderInfo has store {
        _allocate_point: u64,
        _acc_per_share: u128,
    ...
    public struct Rewarder has store {
    ...
    public struct RewarderManager has key, store {
        _id: UID,
        _vault: Bag,
    ...
    public struct InitRewarderManagerEvent has copy, drop {
        _id: ID,
    }

    public struct CreateRewarderEvent has copy, drop {
        _reward_coin: TypeName,
        _emission_per_second: u128,
    }

    public struct UpdateRewarderEvent has copy, drop {
        _reward_coin: TypeName,
        _emission_per_second: u128,
    }
    ...
    public(package) fun register_pool(_manager: &mut RewarderManager, _clmm_pool_id: ID) {
        abort 0
    }

    public(package) fun add_pool<RewardCoin>(
        _manager: &mut RewarderManager,
        _clmm_pool_id: ID,
        _allocate_point: u64,
        _clock: &Clock,
    ...
    public(package) fun set_pool<RewardCoin>(
        _manager: &mut RewarderManager,
        _clmm_pool_id: ID,
        _allocate_point: u64,
        _clock: &Clock,
    ...
    public(package) fun pool_rewards_settle(
        _manager: &mut RewarderManager,
    ...
    public(package) fun set_pool_share(
        _manager: &mut RewarderManager,
        _clmm_pool_id: ID,
        _pool_share: u128,
    ) {
    ...
    public(package) fun withdraw_reward<RewardCoin>(
        _manager: &mut RewarderManager,
        _clmm_pool_id: ID,
        _amount: u64,
